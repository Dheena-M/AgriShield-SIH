...
async function runDetect(fileOverride) {
  ...
  if (output) output.innerHTML = `<div class="card" style="margin-top:16px"><p class="muted"><span class="material-symbols-outlined" style="vertical-align:middle;animation:pulse-dot 1s infinite">sync</span> Analyzing leaf with AgriShield deep-learning models & Qwen2.5 3B Edge AI...</p></div>`;
  const fd = new FormData();
  fd.append("file", file);
  fd.append("crop", crop);
  try {
    ...
    if (output) output.innerHTML = `<div class="card" style="margin-top:14px;border-color:var(--danger)"><p style="color:var(--danger)"><b>Analysis error:</b> ${escapeHtml(err.message)}</p><p class="muted">Please ensure the uploaded file is a valid image (JPG, PNG, WEBP).</p></div>`;
  }
}
